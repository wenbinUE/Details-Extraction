...
const strapiUrl = process.env.STRAPI_API_URL || "http://localhost:1337/api"; // Strapi URL
const strapiAPItoken = process.env.STRAPI_API_TOKEN;

// Strapi API endpoint and headers
const headers = {
...
interface Course {
    ...
    intakes: string;
    ptptn_type: string;
    english_requirements: EnglishRequirement[];
}
...
const extractDetails = async (uniId: string): Promise<void> => {
    ...
    try {
        ...
        while (await cursor.hasNext()) {
            ...
            if (doc.data && Array.isArray(doc.data.level_of_studies)) {
                doc.data.level_of_studies = doc.data.level_of_studies.map(function (
                    lvl: string | ObjectIdType
                ) {
                    if (typeof lvl === "string" && lvl.length === 24)
                        return new ObjectId(lvl);
                    changed = true;
                    return lvl;
                });
            }
            ...
            if (
                ...
                doc.campus_id = new ObjectId(doc.campus_id);
                changed = true;
            }
            ...
            if (
                ...
                doc.university_id = new ObjectId(doc.university_id);
                changed = true;
            }
        ...
        console.error("Connection or Aggregation error:", err);
    } finally {
        client.close();
    }
}
...
async function sendToGoogleSheet(rows: (string | number)[][], sheetName: string) {
    ...
    // Send each row to Strapi
    // for (const row of rows) {
    //     await sendRowToStrapi(row);
    // }
}

async function sendRowToStrapi(row: (string | number)[]) {
  // Map your row to Strapi fields
  const data = {
    data: {
      name: row[5],         // University name
      location: row[1],     // University location
    }
  };

  console.log("Sending to Strapi:", data);
  const res = await axios.post(strapiUrl, data, { headers });
